...
const socket = io();
let currentTaskFilter = 'all'; // Varsayılan filtre: Tümü
...
function filterTaskTable(filterType, btn) {
    ...
    // Filtre tipini kaydet ve tabloyu yenile
...
async function refreshTaskTable() {
    ...
    try {
        ...
        if (currentTaskFilter === 'active') {
            // Bekleyenler: Henüz çözülmemiş olanlar (atandi veya beklemede)
            tasks = tasks.filter(t => t.durum !== 'cozuldu');
        } else if (currentTaskFilter === 'completed') {
            // Çözülenler: Sadece durumu 'cozuldu' olanlar
            tasks = tasks.filter(t => t.durum === 'cozuldu');
        }
        ...
        if (tasks.length === 0) {
            ...
            // Boş durum mesajını duruma göre özelleştirelim
            let msg = 'Kayıt bulunamadı.';
            if (currentTaskFilter === 'active') msg = 'Şu an bekleyen aktif bir iş yok.';
            if (currentTaskFilter === 'completed') msg = 'Henüz tamamlanan bir iş yok.';
        ...
        tasks.forEach(t => {
            ...
            // 5. Aksiyon (Sadece DB standartları)
            let actionContent = '';
            if (t.durum === 'cozuldu') {
